    ...
    if event.type == VkBotEventType.MESSAGE_NEW:
        ...
        if "payload" in event.object.message.keys():
            ...
            if "section" in eval(event.object.message['payload']).keys():
                ...
                if eval(event.object.message['payload'])['section'] == "author":
                    if event.from_user:
                        cache['find author'].append(event.message.from_id)
                        user.find_author(vk, event)
        ...
        # отлов сообщений, не связанных с кнопкой
            ...
            # указывается автор
            elif event.message.from_id in cache['find author']:
                user.author(vk, event, core.db(event.message.text.capitalize()))
                user.start(vk, event)
                cache['find author'].remove(event.message.from_id)
